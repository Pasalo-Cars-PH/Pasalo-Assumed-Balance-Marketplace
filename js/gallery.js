...
(function(){
  ...
  function build(){
    const vehicle=window.vehicles?.find(v=>v.id===getId());
    const photo=root.querySelector('.detail-photo');
    if(!vehicle||!photo||photo.dataset.galleryReady)return;

    const raw=Array.isArray(vehicle.images)&&vehicle.images.length
      ? vehicle.images
    ...
    let index=0;
    const title=String(vehicle.make||'')+' '+String(vehicle.model||'')+' '+String(vehicle.variant||'');

    photo.classList.add('vehicle-gallery');
    photo.innerHTML=`
      <div class="gallery-main">
        <img id="gallery-main-image" src="${esc(urls[0])}" alt="${esc(title)}" loading="eager" decoding="async">
        ...
        <span class="gallery-count" id="gallery-count">1 / ${urls.length}</span>
      </div>
      <div class="gallery-thumbs" role="tablist" aria-label="Vehicle photos">
        ${urls.map((u,i)=>`<button class="gallery-thumb${i===0?' active':''}" type="button" data-index="${i}" aria-label="View photo ${i+1}"><img src="${esc(u)}" alt="" loading="lazy"></button>`).join('')}
      </div>`;
    ...
    const show=i=>{
      ...
      main.src=urls[index];
      main.alt=`${title.trim()} — photo ${index+1}`;
      count.textContent=`${index+1} / ${urls.length}`;
      thumbs.forEach((b,n)=>b.classList.toggle('active',n===index));
