...
function populateMarketplaceFilters(){
  ...
  fillSelect('#make',[...new Set(active.map(v=>v.make).filter(Boolean))].sort(),'Any brand');
  fillSelect('#location',[...new Set(active.map(v=>v.location).filter(v=>v&&v!=='To confirm'))].sort(),'Any location');
  fillSelect('#body',[...new Set(active.map(v=>v.bodyType).filter(Boolean))].sort(),'Any type');
}

function renderListings(){
  const grid=document.querySelector('#results-grid');if(!grid)return;
  populateMarketplaceFilters();
  const qEl=document.querySelector('#q'),makeEl=document.querySelector('#make'),locEl=document.querySelector('#location'),bodyEl=document.querySelector('#body'),monthlyEl=document.querySelector('#monthly'),cashEl=document.querySelector('#cash'),minMonthlyEl=document.querySelector('#min-monthly'),minCashEl=document.querySelector('#min-cash'),sortEl=document.querySelector('#sort');
  if(qEl)qEl.value=qs('q')||'';if(makeEl)makeEl.value=qs('make')||'';if(locEl)locEl.value=qs('location')||'';if(bodyEl)bodyEl.value=qs('body')||'';if(monthlyEl)monthlyEl.value=qs('monthly')||'';if(cashEl)cashEl.value=qs('cash')||'';if(minMonthlyEl)minMonthlyEl.value=qs('minMonthly')||'';if(minCashEl)minCashEl.value=qs('minCash')||'';if(sortEl)sortEl.value=qs('sort')||'new';
  const getList=()=>{
    const q=(qEl?.value||'').trim().toLowerCase(),make=makeEl?.value||'',loc=locEl?.value||'',body=bodyEl?.value||'',maxMonthly=Number(monthlyEl?.value)||Infinity,maxCash=Number(cashEl?.value)||Infinity,minMonthly=Number(minMonthlyEl?.value)||0,minCash=Number(minCashEl?.value)||0;
    let list=vehicles.filter(v=>{
      const hay=`${v.make} ${v.model} ${v.variant} ${v.color} ${v.location}`.toLowerCase();
      const m=v.monthly==null?Infinity:numeric(v.monthly),c=v.cashOut==null?Infinity:numeric(v.cashOut);
      return v.status==='active'&&(!q||hay.includes(q))&&(!make||v.make===make)&&(!loc||v.location===loc)&&(!body||v.bodyType===body)&&m<=maxMonthly&&c<=maxCash&&m>=minMonthly&&c>=minCash;
    });
    const sort=sortEl?.value||'new';
    if(sort==='monthly')list.sort((a,b)=>(a.monthly??Infinity)-(b.monthly??Infinity));
    else if(sort==='cash')list.sort((a,b)=>numeric(a.cashOut||Infinity)-numeric(b.cashOut||Infinity));
    else if(sort==='year')list.sort((a,b)=>(Number(b.year)||0)-(Number(a.year)||0));
    else list.sort((a,b)=>new Date(b.listedAt)-new Date(a.listedAt));
    return list;
  };
  const draw=()=>{
    const list=getList(),count=document.querySelector('#result-count'),summary=document.querySelector('#result-summary'),wrap=document.querySelector('#load-more-wrap'),more=document.querySelector('#load-more'),moreCount=document.querySelector('#load-more-count'),chips=document.querySelector('#active-filters');
    const shown=list.slice(0,visibleCount);
    if(count)count.textContent=`${list.length} vehicle${list.length===1?'':'s'} found`;
    if(summary)summary.textContent=list.length?`Showing ${shown.length} of ${list.length} available listings`:'Try a wider budget or remove a filter.';
    grid.innerHTML=list.length?shown.map(card).join(''):'<div class="empty"><strong>No matching vehicles yet.</strong><br>Try increasing your maximum monthly/cash-out or clearing a filter.</div>';
    if(wrap){wrap.classList.toggle('hidden',shown.length>=list.length||!list.length);}
    if(moreCount)moreCount.textContent=shown.length<list.length?`${list.length-shown.length} more available`:'';
    if(chips){const labels=[];if(qEl?.value)labels.push(`Search: ${qEl.value}`);if(makeEl?.value)labels.push(makeEl.value);if(locEl?.value)labels.push(locEl.value);if(bodyEl?.value)labels.push(bodyEl.value);if(minMonthlyEl?.value)labels.push(`Monthly ≥ ₱${numeric(minMonthlyEl.value).toLocaleString()}`);if(monthlyEl?.value)labels.push(`Monthly ≤ ₱${numeric(monthlyEl.value).toLocaleString()}`);if(minCashEl?.value)labels.push(`Cash-out ≥ ₱${numeric(minCashEl.value).toLocaleString()}`);if(cashEl?.value)labels.push(`Cash-out ≤ ₱${numeric(cashEl.value).toLocaleString()}`);chips.innerHTML=labels.map(x=>`<span class="filter-chip">${escapeHtml(x)}</span>`).join('')}
  };
  const applyUrl=()=>{const u=new URL(location.href);[['q',qEl],['make',makeEl],['location',locEl],['body',bodyEl],['monthly',monthlyEl],['cash',cashEl],['minMonthly',minMonthlyEl],['minCash',minCashEl],['sort',sortEl]].forEach(([k,el])=>{const v=el?.value||'';if(v)u.searchParams.set(k,v);else u.searchParams.delete(k)});history.replaceState({},'',u)};
  document.querySelector('#apply')?.addEventListener('click',()=>{visibleCount=9;applyUrl();draw()});
  document.querySelector('#clear-filters')?.addEventListener('click',()=>{[qEl,makeEl,locEl,bodyEl,monthlyEl,cashEl,minMonthlyEl,minCashEl].forEach(el=>{if(el)el.value=''});if(sortEl)sortEl.value='new';visibleCount=9;applyUrl();draw()});
  sortEl?.addEventListener('change',()=>{visibleCount=9;applyUrl();draw()});
  document.querySelector('#load-more')?.addEventListener('click',()=>{visibleCount+=9;draw()});
  draw();
}
...
setInterval(()=>{document.querySelectorAll('.vehicle-meta').forEach(el=>{const spans=el.querySelectorAll('span');spans.forEach(p=>{const text=p.textContent||'';if(text.includes('Listed ')){const v=vehicles.find(x=>text.includes(title(x)));if(v)p.textContent=`🕒 ${age(v)}`}})});},60000);
